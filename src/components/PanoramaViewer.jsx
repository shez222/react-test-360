...
const PanoramaViewer = () => {
  ...
  useEffect(() => {
    ...
    if (window.DeviceOrientationEvent) {
        ...
        DeviceOrientationEvent.requestPermission()
          .then(permissionState => {
            if (permissionState === 'granted') {
              ...
              setInstructions("Device orientation permission denied. Using manual controls.");
            }
          })
          .catch(() => {
            // Fallback to OrbitControls on error
            controls = new OrbitControls(camera, renderer.domElement);
            setupOrbitControls(controls);
            setInstructions("Device orientation not supported. Using manual controls.");
          });
      } else {
        // Non iOS devices
        controls = new DeviceOrientationControls(camera);
      ...
      // Use OrbitControls for desktop or unsupported devices
      controls = new OrbitControls(camera, renderer.domElement);
      setupOrbitControls(controls);
    }

    // Helper function to setup OrbitControls
    function setupOrbitControls(controlsInstance) {
    ...
    // Create a Video Texture once the video is ready
    videoReady.then(() => {
      ...
      const placeObjectOnSphere = (obj, angle) => {
        ...
        obj.lookAt(new THREE.Vector3(0, 0, 0)); // Ensure the object faces the center
      };
    ...
    const animate = () => {
      ...
      if (
        ...
        isMarkerCentered(cameraRef.current, markerRef.current)
      ) {
        capturing = true;
  ...
  const performCapture = (isAuto) => {
    ...
    const video = videoTextureRef.current ? videoTextureRef.current.image : null;
